@extends('admin.layouts.app') {{-- Sesuaikan dengan nama file layout utama Sneat kamu --}}
...
<div class="container-xxl flex-grow-1 container-p-y">
    ...
    <div class="card">
        <div class="card-header d-flex justify-content-between align-items-center">
            <h5 class="mb-0">Daftar Berita SMK Negeri 11</h5>
            ...
            <table class="table table-hover">
                ...
                <tbody class="table-border-bottom-0">
                    ...
                    <tr>
                        ...
                        <td>
                            @if($berita->gambar)
                                <img src="{{ asset('storage/' . $berita->gambar) }}" alt="Gambar" class="rounded" width="60">
                            @else
                                <span class="badge bg-label-secondary">No Image</span>
                            @endif
                        </td>
                        <td><strong>{{ Str::limit($berita->judul, 40) }}</strong></td>
                        <td>{{ $berita->kategori }}</td>
                        <td>
                            <span class="badge {{ $berita->status == 'Published' ? 'bg-label-success' : 'bg-label-warning' }}">
                                {{ $berita->status }}
                            </span>
                        </td>
                        ...
                        <td>
                            <div class="d-flex gap-2">
                                <a class="btn btn-sm btn-outline-warning" href="{{ route('berita.edit', $berita->id) }}">
